...
この先、コマンドを実行する場面がたびたび出てくる。それらはすべてターミナル
（端末）——コマンドを打ち込んでコンピュータに直接指示を出すアプリケーション——
の中で実行している。打ち込んだ文字列を解釈し、実際にOSへプロセスの起動を
依頼するプログラムをシェルと呼ぶ（Macの標準はzshやbash）。「コマンドを打つ」とは、
シェルに「このプログラムを、この引数で実行してくれ」と伝えていること。
...
- `python3 -m http.server 8000` — `-m`はPythonに「後ろの名前をモジュールとして
  ...
  ポート番号。
- 末尾の`&` — このコマンドを「バックグラウンド」で動かす指示。シェルは通常、実行した
  コマンドが終わるまで次の入力を受け付けないが、`&`を付けるとコマンドを裏で走らせた
  まま、すぐ次のコマンドを打てるようになる。
- `nohup` — 端末を閉じるとシェルは動いているプロセスに終了の指示を送るが、`nohup`は
  そのプロセスに「その指示を無視して動き続けろ」と伝える仕組み。`&`だけだと端末を
  閉じた瞬間にサーバーも道連れで落ちる。
...
- `disown` — バックグラウンドで動かしたジョブを、今のシェルの管理下から切り離す。
  切り離さないと、シェルを閉じたときに一緒に終了の指示が送られることがある。
...
動かしている端末で`Ctrl+C`を押す（シェルからそのプロセスへ「止まってくれ」という
指示が送られる）か、`kill`コマンドでプロセスの番号を指定して同じ指示を送るか。
今回は`nohup`で端末から切り離しているので、終了させるには`kill`を使う。
