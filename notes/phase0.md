...
シェルのジョブテーブルと、OSが管理するプロセス一覧は別物。実際に見比べると
違いが分かる。
...
`jobs`はシェルの組み込みコマンドで、**今のシェルセッションが管理しているジョブ**
だけを表示する。別のターミナルを開き直した直後や、`disown`した後のジョブは、
ここには出てこない。一方`ps`はOSカーネルが管理する、**システム全体の全プロセス**を
一覧するコマンドで、シェルのセッションとは無関係に、動いているプロセスをPID
（プロセスごとに振られる識別番号）付きで見つけられる。`kill`でプロセスを終了
させるときに指定するのもこのPID。
...
シグナルが送られる）か、`kill`コマンドでPIDを指定する（デフォルトで`SIGTERM`
という終了シグナルが送られる）か。`SIGHUP`・`SIGINT`・`SIGTERM`は
