...
システム全体で動いている全プロセスを見るコマンドが`ps`——OSカーネルが管理する
...
1. コマンドライン引数を解析する。ポート番号のほか、応答するプロトコル
   バージョンもここで決まる——デフォルトは`HTTP/1.0`だが、実は`-p HTTP/1.1`
   のように指定して変えられる。前節で「常にHTTP/1.0に固定されている」と
   書いたのは、正確には「デフォルトが1.0」だった。
2. TCPソケットを作り、`bind()`でポート8000に紐付け、`listen()`で接続を
   受け付けられる状態にする——これが前節で説明した「listen」の実体。
...
`python3 -m http.server`はデフォルトで`ThreadingHTTPServer`を使う。これは
1接続ごとに新しいスレッドを1本立てて処理を任せるサーバーで、複数のリクエスト
が同時に来ても並行して捌ける。

リクエストの1行目（`GET / HTTP/1.1`）を読んだ後は、`"do_" + メソッド名`と
いう名前のメソッドを探して呼び出す仕組みになっている。`GET`なら`do_GET`、
`POST`なら`do_POST`——今回のアクセスで実際に呼ばれたのは`do_GET`。

**このサーバーがどんなサーバーか**

`do_GET`（`SimpleHTTPRequestHandler`が持つ実装）は、リクエストされたパスに
対応するファイルをディスク上で探し、見つかればその中身を、見つからなければ
404を返す。つまりこれは「ファイルシステム上のファイルをそのまま配信する」
...
`* Closing connection`で接続を切っている。HTTP/1.1同士なら`Connection:
keep-alive`で同じTCP接続を使い回せる場面だが、サーバーが1.0で応答した以上、
クライアント側もそれに合わせて「1回限りの接続」として扱う。バージョンの表示は
飾りではなく、「このやり取りをどちらの流儀で進めるか」を決める実質的な情報。
